...
import com.lephiha.do_an.CallVideo.stringee.service.RejectCallReceiver;
...
public class NotificationUtils {
    ...
    private String createCallNotificationChannel() {
        int channelIndex = PrefUtils.getInstance(context).getInt(Constant2.PREF_INCOMING_CALL_CHANNEL_ID_INDEX, 0);
        String channelId = Constant2.INCOMING_CALL_CHANNEL_ID + channelIndex;
        if (VERSION.SDK_INT >= VERSION_CODES.O) {
            //delete old channel
            ...
            if (channel != null && !(channel.getImportance() == NotificationManager.IMPORTANCE_MAX || channel.getImportance() == NotificationManager.IMPORTANCE_HIGH)) {
                ...
                channelId = Constant2.INCOMING_CALL_CHANNEL_ID + channelIndex;
            }
            //create new channel
            channel = new NotificationChannel(channelId, Constant2.INCOMING_CALL_CHANNEL_NAME, NotificationManager.IMPORTANCE_HIGH);
            PrefUtils.getInstance(context).putInt(Constant2.PREF_INCOMING_CALL_CHANNEL_ID_INDEX, channelIndex);
            channel.setDescription(Constant2.INCOMING_CALL_CHANNEL_DESC);
    ...
    public void showIncomingCallNotification(String from, boolean isStringeeCall, boolean isVideoCall) {
        ...
        fullScreenIntent.putExtra(Constant2.PARAM_IS_VIDEO_CALL, isVideoCall);
        fullScreenIntent.putExtra(Constant2.PARAM_IS_INCOMING_CALL, true);
        fullScreenIntent.putExtra(Constant2.PARAM_IS_STRINGEE_CALL, isStringeeCall);
        ...
        actionAnswerIntent.putExtra(Constant2.PARAM_IS_VIDEO_CALL, isVideoCall);
        actionAnswerIntent.putExtra(Constant2.PARAM_IS_INCOMING_CALL, true);
        actionAnswerIntent.putExtra(Constant2.PARAM_IS_STRINGEE_CALL, isStringeeCall);
        actionAnswerIntent.putExtra(Constant2.PARAM_ACTION_ANSWER_FROM_PUSH, true);
        ...
        actionRejectIntent.putExtra(Constant2.PARAM_IS_VIDEO_CALL, isVideoCall);
        actionRejectIntent.putExtra(Constant2.PARAM_IS_INCOMING_CALL, true);
        actionRejectIntent.putExtra(Constant2.PARAM_IS_STRINGEE_CALL, isStringeeCall);
        ...
        nm.notify(Constant2.INCOMING_CALL_ID, incomingCallNotification);
    }

    private void createMediaServiceChannel() {
        if (VERSION.SDK_INT >= VERSION_CODES.O) {
            NotificationChannel channel = new NotificationChannel(Constant2.MEDIA_CHANNEL_ID, Constant2.MEDIA_CHANNEL_NAME, NotificationManager.IMPORTANCE_DEFAULT);
            channel.setDescription(Constant2.MEDIA_CHANNEL_DESC);
            channel.setSound(null, null);
            nm.createNotificationChannel(channel);
        }
    }
    ...
    public Notification createMediaNotification() {
        ...
        Builder builder = new Builder(context, Constant2.MEDIA_CHANNEL_ID);
